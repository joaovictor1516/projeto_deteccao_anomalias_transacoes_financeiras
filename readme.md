# Projeto de Deteccão de Anomálias nas Transações Financeiras
## Introdução:
Este é o projeto de conclusão do módulo Análise de Dados com Python: da Preparação à Aplicação com Segurança. Nele foi proposta a criação de um modelo simples para a detecção de fraudes bancárias usando como base de dados o modelo opensource da tensorflow chamado creditcard.
...
- ### Undersampling:
    - Ele descarta a quantidade a mais de não fraudes.
- ### Oversampling:
...
## Conclusão:
Este projeto nos ajudou a visualizar de forma analítica que não há um modelo melhor que outro em todos os casos, a famosa bala de prata, já que dependendo do problema a ser solucionado devemos usar diferentes modelos e diferentes formas de balanceamento.
Observamos que os dois modelos tiveram o melhor Recall no balanceamento undersampling mas esta estratégia também gerou o menor precision, tornando os modelos inviáveis de uso, e isso ocorreu pelo fato da baixa quantidade de dados de treinamento. Já em relação ao balanceamento oversampling tivemos o Recall do Logistic Regression um pouco maior, mas com uma precisão bem menor que a do modelo Random Forest. E por fim nos modelos sem balanceamento tivemos uma boa precisão de hambos, e com um Recall aceitável para o Random Forest enquanto que o do Logistic Regression estava muito próximo do 0.5, mostrando que o modelo estava com uma alta aleatoriedade.
Por fim o adendo de que este estudo serve apenas como uma comparação entre os modelos e não representa a melhor solução para o problema, pois para isso podemos usar o GridSearchCV para encontrar a melhor combinação de métricas para o modelo além de ser altamente recomendado o uso do modelo Xboost, que é um conjunto de árvores de decisão capazes de corrigir o erro das árvores anteriores, mas foram escolhidos os modelos Logistic Regression e Random Forest por serem amplamente conhecidos e o modelo Xboost precisar de mais hardware.